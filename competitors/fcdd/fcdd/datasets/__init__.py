...
DS_CHOICES = ('mnist', 'cifar10', 'fmnist', 'mvtec', 'imagenet', 'pascalvoc', 'custom', 'custom_our', 'custom_our_mvtec')
PREPROC_CHOICES = (
...
def load_dataset(dataset_name: str, data_path: str, normal_class: int, preproc: str,
    ...
    elif dataset_name == 'custom_our_mnist':
        print('CUSTOM_OUR')
    ...
    elif dataset_name == 'custom_our_mvtec':
        print('CUSTOM_OUR_MVTEC')
        dataset = ADImageDatasetGTM(
            root=data_path, normal_class=normal_class, preproc=preproc,
            supervise_mode=supervise_mode, noise_mode=noise_mode, online_supervision=online_supervision,
            oe_limit=oe_limit, logger=logger, nominal_label=nominal_label, shape=(3, 224, 224)
        )
    else:
