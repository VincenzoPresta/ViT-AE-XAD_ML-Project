...
from tools.create_dataset import wfdd
...
if not os.path.exists(f'PLOT/{dataset}_mul'):
    os.makedirs(f'PLOT/{dataset}_mul')

if not os.path.exists(f'results/AEXAD/{ds}'):
    os.makedirs(f'results/AEXAD/{ds}')

for c in range(4):
    if not os.path.exists(f'PLOT/{dataset}_sum_aug/{c}'):
        os.makedirs(f'PLOT/{dataset}_sum_aug/{c}')
    #if not os.path.exists(f'results/AEXAD/{ds}_mul/{c}'):
    #    os.makedirs(f'results/AEXAD/{ds}_mul/{c}')
    #if not os.path.exists(f'results/AEXAD/{ds}_mul/{c}/{s}'):
    #    os.makedirs(f'results/AEXAD/{ds}_mul/{c}/{s}')


    X_train, Y_train, X_test, Y_test, GT_train, GT_test = wfdd(c, 'datasets/WFDD', na, shape=img_size, seed=s)
    ...
    ret_path = f'results/{ds}_vgg_upsample_small_mask_sum_aug/{c}/{s}'
    ...
    heatmaps_aexad_norm_blur = blurred_htmaps(heatmaps_aexad_norm, scale=0.5)
    ...
    def plots(i, ids_anomalies, x, gt, hs1, hs2, hs3, xaucs1, xaucs2, xaucs3, num_std=None, c=None):
        ...
        if num_std == None:
            if c == None:
                name = f'PLOT/{dataset}_mul/fig_{i}.jpg'
            else:
                name = f'PLOT/{dataset}_mul/{c}/fig_{i}.jpg'
        else:
            if c == None:
                name = f'PLOT/{dataset}_mul/fig_{i}_{num_std}.jpg'
            else:
                name = f'PLOT/{dataset}_mul/{c}/fig_{i}_{num_std}.jpg'
        plt.savefig(name)
