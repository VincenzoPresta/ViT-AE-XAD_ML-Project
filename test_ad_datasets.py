...
from tools.create_dataset import extract_dataset, load_dataset
...
import sys
sys.path.extend(['/media/rosariaspada/Dati/AE-XAD-v2', '/media/rosariaspada/Dati/AE-XAD-v2/competitors/deviation', '/media/rosariaspada/Dati/AE-XAD-v2/competitors/fcdd', '/media/rosariaspada/Dati/AE-XAD-v2/competitors/bgad'])
...
def class0(x): return torch.from_numpy(np.array([0]))
def class1(x): return torch.from_numpy(np.array([1]))
...
preprocresh = transforms.Compose([
    ...
    transforms.Resize((img_shape, img_shape), Image.NEAREST),
    # ------------------------
...
dataset = f'road_inspection'

ret_path = f'results/{dataset}_final_f2/'
if not os.path.exists(ret_path):
    os.makedirs(ret_path)

if not os.path.exists(f'PLOT/{dataset}_final_f2'):
    os.makedirs(f'PLOT/{dataset}_final_f2')


dataset_path = f'datasets/{dataset}/'

X_train, Y_train, X_test, Y_test, GT_train, GT_test = load_dataset(f'datasets/{dataset}')
...
X_train_resh = np.zeros((X_train.shape[0], 3, img_shape, img_shape))
GT_train_resh = np.zeros((GT_train.shape[0], 3, img_shape, img_shape))
X_test_resh = np.zeros((X_test.shape[0], 3, img_shape, img_shape))
GT_test_resh = np.zeros((GT_test.shape[0], 3, img_shape, img_shape))
for i in range(X_train.shape[0]):
    X_train_resh[i, :, :, :] = np.array(preproc1(X_train[i, :, :, :])).swapaxes(2, 1).swapaxes(1, 0)
    GT_train_resh[i, :, :, :] = np.array(preprocresh(GT_train[i, :, :, :])).swapaxes(2, 1).swapaxes(1, 0)
for i in range(X_test.shape[0]):
    X_test_resh[i, :, :, :] = np.array(preproc1(X_test[i, :, :, :])).swapaxes(2, 1).swapaxes(1, 0)
    GT_test_resh[i, :, :, :] = np.array(preprocresh(GT_test[i, :, :, :])).swapaxes(2, 1).swapaxes(1, 0)
...
del X_train, Y_train, GT_train, X_train_resh, GT_train_resh, X_test_resh, GT_test_resh,

tracker = EmissionsTracker(tracking_mode='process', log_level='critical', allow_multiple_runs=True)
tracker.start_task('fcdd')
t = time.time()
heatmaps_fcdd, scores_fcdd, gtmaps_fcdd, labels_fcdd, time_fcdd = launch_fcdd(data_path, epochs=200, batch_size=16) #200
times.append(time.time() - t)
emissions.append(tracker.stop_task('fcdd').emissions)

tracker.start_task('aexad')
t = time.time()
heatmaps_aexad, scores_aexad, _, _, time_aexad = launch_aexad(data_path, 200, 16, 64, None, None, f, ae_type,
                                                      save_intermediate=False, save_path=ret_path, use_cuda=True,
                                                      dataset='tf_ds', loss='aexad', steps_per_epoch=5) # 400
times.append(time.time() - t)
emissions.append(tracker.stop_task('aexad').emissions)

tracker.start_task('bgad')
t = time.time()
#heatmaps_devnet, scores_devnet, gtmaps_devnet, labels_devnet, tot_time_devnet = launch_deviation(dataset_root=data_path, epochs=50, img_size=448)
...
t = time.time()
heatmaps_ae, scores_ae, _, _, time_ae = launch_aexad(data_path, 200, 16, 64, None, None, f, ae_type,
                                                      save_intermediate=False, save_path=ret_path, use_cuda=True,
...
heatmaps_aexad_norm = np.where(heatmaps_aexad > 1, 1., heatmaps_aexad)
...
def plots(i, ids_anomalies, x,gt,hs1, hs2, hs3, xaucs1, xaucs2, xaucs3, num_std=None):
    id  = ids_anomalies[i]
    plt.subplot(1, 5, 1)
    ...
    if num_std == None:
        name = f'PLOT/{dataset}_final_f2/fig_{i}.jpg'
    else:
        name = f'PLOT/{dataset}_final_f2/fig_{i}_{num_std}.jpg'
    plt.savefig(name)


GT_test = GT_test[:, 0] / 255.

xaucs_aexad = Xaucs(GT_test, heatmaps_aexad_norm_blur, ids_anomalies)
xaucs_fcdd = Xaucs(GT_test, heatmaps_fcdd, ids_anomalies)
#xaucs_devnet = Xaucs(np.array(gtmaps_devnet)[:, 0]//255, np.array(heatmaps_devnet))
xaucs_bgad = Xaucs(GT_test, heatmaps_bgad, ids_anomalies)
xaucs_ae = Xaucs(GT_test, heatmaps_ae, ids_anomalies)
...
for i in range(Y_test[Y_test==1].shape[0]):
    plots(i, ids_anomalies, imgs.transpose((0, 2, 3, 1)), GT_test,
          ...
          xaucs_aexad, xaucs_fcdd, xaucs_bgad)
...
for num_std in np.arange(1., 4., 0.5):
    ...
    for i in range(ht_aexad_bin[Y_test == 1].shape[0]):
        ...
        iou_fcdd[i] = iou(gtmaps_fcdd[Y_test == 1][i, 0], ht_fcdd_bin[Y_test == 1][i])
        pre_fcdd[i] = precision(gtmaps_fcdd[Y_test == 1][i, 0], ht_fcdd_bin[Y_test == 1][i])
        rec_fcdd[i] = recall(gtmaps_fcdd[Y_test == 1][i, 0], ht_fcdd_bin[Y_test == 1][i])
...
for i in range(Y_test[Y_test==1].shape[0]):
    plots_std_bin(i, ids_anomalies, imgs.transpose((0, 2, 3, 1)), GT_test,
          imgs_aexad, imgs_ae, imgs_fcdd, imgs_bgad, np.arange(1., 4., 0.5), dataset)

d['bin_stats'] = d_acc

with open(f'{ret_path}/stats.txt', 'w') as file:
